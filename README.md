...
This is the foundation project for a course that will extend it in later lessons with Ragas evaluation, Redis conversation memory, hybrid search, and production monitoring. The architecture is deliberately built so those additions are new files and new modules, not rewrites of existing ones.

---

## Read this first if you are new to RAG

If the terms below are unfamiliar, read this section before the setup steps. Understanding these five ideas is what matters most for explaining this project, more than any specific line of code.

**1. Why raw text needs chunking.**
A whole PDF is too large and too unfocused to hand to an AI model as "the answer is somewhere in here." Chunking breaks a document into smaller, topic-sized pieces, so that later, only the few pieces relevant to a specific question get used, instead of the entire document.

**2. What an embedding is.**
An embedding is a list of numbers (a vector) that represents the meaning of a piece of text. Texts with similar meaning end up with similar (mathematically close) vectors, even if they use completely different words. This is what allows a computer to compare meaning instead of just comparing exact words.

**3. What a vector database does with those embeddings.**
ChromaDB stores every chunk's embedding, alongside the original text and metadata (which file it came from, which section, and so on). Its job is to answer, very quickly, "which stored embeddings are closest to this new embedding?" That is the entire mechanism behind semantic search.

**4. What "retrieval" means at query time.**
When a question comes in, it gets embedded using the same model used for the documents, so the question and the documents live in the same vector space. ChromaDB is searched for the closest matching chunks. Those chunks, not the whole document, are handed to the language model as context.

**5. Why this specific chunking and retrieval design.**
Chunks are split by heading first, then by sentence, with a small overlap between consecutive chunks. This avoids blending two unrelated topics into one chunk, avoids cutting a sentence in half, and reduces the chance that a fact sitting exactly at a chunk boundary gets lost. Retrieval then returns the top 5 most similar chunks, which are assembled into one context block and handed to Gemini with a strict instruction: answer only from that context, and say so plainly if the answer is not there.
...
- `chat/chat.service.ts`: orchestrates the RAG flow (embed question, search, build context, generate answer). This is the "RAG Service" the project brief describes.
...
Being upfront about these is more credible in an interview than pretending the project has none:

- **Embeddings are generated one chunk at a time**, in a loop, rather than batched into a single API call. Fine for portfolio-scale documents; a production version would batch requests to reduce latency and API call count.
...
- **Retrieval is pure vector similarity search**, with no keyword/BM25 component and no reranking step. Hybrid search and reranking are explicitly left for a later stage (see below).
- **No conversation memory.** Each question is answered independently; the chat endpoint does not use prior turns as context. Redis-backed memory is a planned future lesson.

## Designed for future extensibility

Because the vector store sits behind `VectorStorePort`, and the Gemini services sit behind their own single-purpose classes, these can be added later without modifying `ChatService` or `KnowledgeService`'s core flow:

- Ragas evaluation (a new module that runs alongside `ChatService`, not inside it)
- Redis conversation memory (a new service injected into `ChatService`, extending its inputs)
- Swapping ChromaDB for Pinecone or another vector database (a new class implementing `VectorStorePort`)
- Hybrid search or metadata filtering (extending `VectorStorePort.query` or adding a second query method)
- Streaming responses, authentication, multiple users, rate limiting, caching, and observability, each isolated to their own module or middleware
